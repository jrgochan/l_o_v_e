...
fixes: List[Tuple[str, int, str, str]] = [
    ...
    ("app/services/aggregate_emotion_service.py", 420, "return round(min(complexity, 1.0), 3)", "return cast(float, round(min(complexity, 1.0), 3))"),
    ("app/services/aggregate_emotion_service.py", 466, "return round(min(clarity, 1.0), 3)", "return cast(float, round(min(clarity, 1.0), 3))"),
    
    # arg-type fixes - simple conversions
    ("app/services/recommendation_engine.py", 742, 'journey["emotions"]', 'list(journey["emotions"])'),
