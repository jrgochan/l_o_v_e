...
from app.services.ollama_manager import ModelInfo, ModelDetails
...
def mock_ollama():
    with patch.object(ai_models, "OllamaManager", autospec=True) as mock, \
         patch("app.api.routes.ai_models.asyncio.sleep", new_callable=AsyncMock):
...
async def test_stream_pull_progress_success(mock_ollama):
    ...
    with patch.dict(ai_models.active_pulls, {
        task_id: {
...
async def test_stream_pull_websocket_disconnect_direct(mock_ollama):
    ...
    from app.api.routes.ai_models import stream_pull_progress, active_pulls
...
async def test_stream_pull_generic_error_direct(mock_ollama):
    ...
    # catch block: await websocket.send_json({"task_id": task_id, "status": "error", "error": str(e)})
