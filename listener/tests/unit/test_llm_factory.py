...
def test_get_llm_vertex_success():
    ...
    with patch("app.config.settings.AI_PROVIDER", "google_vertex"), \
         ...
         patch("app.services.llm_factory.ChatVertexAI"): # Mock the class to avoid init errors
...
def test_get_llm_vertex_model_override():
    ...
    with patch("app.config.settings.AI_PROVIDER", "google_vertex"), \
         ...
         # We can't easily assert exactly without knowing settings value, but we know it should NOT be None
...
def test_vertex_import_logic():
    ...
    # To truly hit the 'except ImportError' line during execution, we'd need to simulate the library missing.
