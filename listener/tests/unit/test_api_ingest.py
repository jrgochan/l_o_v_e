...
def mock_services():
    ...
    with patch('app.services.transcription.get_transcription_service') as mock_trans, \
        ...
        mock_sem.return_value.analyze = AsyncMock(return_value=MagicMock(
            primary_emotion="Joy", 
            category="Positive", 
        ...
        multi_resp.emotions = [MagicMock(emotion_name="Joy", category="Pos", vac=MagicMock(valence=1, arousal=0, connection=0), confidence=0.9, prominence="primary")]
        ...
        mock_multi.return_value.analyze_three_way = AsyncMock(return_value={
            "content_only": multi_resp,
            "voice_only": multi_resp,
            "blended": multi_resp,
            "discrepancy": {"content_voice_distance": 0.0, "flags": [], "interpretation": "ok", "content_primary": "Joy", "voice_primary": "Joy", "blended_primary": "Joy"}
        })
...
def test_ingest_both_inputs():
    response = client.post("/listener/ingest", 
...
def test_ingest_text_success(mock_redis_pool):
    response = client.post("/listener/ingest", 
                           data={"text": "Hello world", "user_id": "u", "session_id": "s"})
    assert response.status_code == 200
    assert response.json()["status"] == "queued"
    mock_redis_pool.return_value.enqueue_job.assert_called_once()
...
def test_analyze_audio_sync(mock_services, mock_aiofiles):
    response = client.post("/listener/analyze-audio", 
                           files={"audio": ("test.wav", b"data", "audio/wav")})
    assert response.status_code == 200
...
def test_analyze_audio_sync_fail(mock_services, mock_aiofiles):
    mock_services["transcription"].return_value.transcribe.side_effect = Exception("Transcribe fail")
...
def test_analyze_audio_multi_emotion(mock_services, mock_aiofiles):
    response = client.post("/listener/analyze-audio-multi-emotion", 
                           files={"audio": ("test.wav", b"data", "audio/wav")})
    assert response.status_code == 200
