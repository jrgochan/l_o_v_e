...
class TestMultiEmotionAnalyzer:
    ...
    async def test_refresh_prompts_error(self, analyzer):
        ...
        with patch("app.services.multi_emotion_analyzer.get_model_fetcher", side_effect=Exception("Fail")):
             await analyzer._refresh_prompts()
    ...
    def test_filter_emotions_edge_cases(self, analyzer):
        ...
        emotions = [{"confidence": 0.1, "emotion_name": "LowConf", "category": "Test", "vac": {}, "prominence": "primary"}]
        with pytest.raises(ValueError, match="No emotions meet minimum confidence"):
            analyzer._filter_and_validate_emotions(emotions)

        # 2. More than 3 emotions (should limit)
        high_conf_emotions = [
             {"confidence": 0.9, "emotion_name": f"E{i}", "category": "Test", "vac": {}, "prominence": "primary"} 
             for i in range(5)
        ]
    ...
    def test_process_llm_markdown_cleaning_explicit(self, analyzer):
        ...
        # Valid JSON payload that passes validation
        # Must align with DetectedEmotionResponse: emotion_name (not name), prominence (not intensity)
        # Relationship must use correct emotion names
        valid_json_payload = '{"emotions": [{"emotion_name": "Joy", "category": "Happiness", "confidence": 0.9, "prominence": "primary", "verification_source": "text", "blend_status": "pure", "vac": {"valence": 0.8, "arousal": 0.5, "connection": 0.5}}], "relationships": [], "complexity_score": 0.5, "emotional_clarity": 0.8, "temporal_pattern": "concurrent", "reasoning": "Test reasoning"}'
    ...
    async def test_process_llm_response_missing_emotions_fails(self, analyzer):
        ...
        json_payload = '{"relationships": [], "complexity_score": 0.5, "emotional_clarity": 0.8, "temporal_pattern": "concurrent", "reasoning": "r"}'
    ...
    async def test_process_llm_response_missing_vac_fails(self, analyzer):
        ...
        json_payload = '{"emotions": [{"emotion_name": "Joy", "category": "Happiness", "confidence": 0.9, "prominence": "primary", "verification_source": "text", "blend_status": "pure"}], "relationships": [], "complexity_score": 0.5, "emotional_clarity": 0.8, "temporal_pattern": "concurrent", "reasoning": "r"}'
    ...
    def test_clinical_flags_coverage(self, analyzer):
        ...
        # We need to test specific branches in _generate_clinical_flags (nested in analyze_three_way)
        # But analyze_three_way calls private _process_llm_response locally...
        # It's hard to test _generate_clinical_flags directly as it is inside analyze_three_way logic?
        # Actually it's part of analyze_three_way large method?
        ...
        with patch.object(analyzer, "_analyze_content_only") as mock_content, \
             ...
             # Blended
             mock_analyze.return_value.aggregate_vac = VACVector(valence=0.0, arousal=0.5, connection=0.5)
             ...
             result = asyncio.run(analyzer.analyze_three_way(text="text", prosody_features={"pitch": 100.0}))
    ...
    def test_process_llm_response_relationships_and_missing_keys(self, analyzer):
        ...
        # If Pydantic requires it, this logic assumes result_dict is raw dict before model validation?
    ...
    def test_process_llm_response_missing_optional_keys(self, analyzer):
        ...
        json_str = json.dumps({
            "emotions": [{
                "emotion_name": "E", "confidence": 0.9, "prominence": "primary", 
                "category": "C", "vac": {"valence": 0, "arousal": 0, "connection": 0}
            }],
            "aggregate_vac": {"valence": 0, "arousal": 0, "connection": 0},
            "temporal_pattern": "concurrent",
            "reasoning": "r",
            "complexity_score": 0.5, # We want to test missing? 
            "emotional_clarity": 0.5
        })
        ...
        # If they are required, then `if "complexity_score" in result_dict` checks are ALWAYS true for valid input.
        # Coverage only misses if we have invalid input that somehow bypasses validation?
        # NO. The checks run BEFORE Pydantic validation (line 561 loads json, checks run 567-873, validation 876).
        # Ah! So we can pass a dict WITHOUT keys, logic runs (and skips `if`), then it crashes at `MultiEmotionAnalysisResponse(**result_dict)`.
        ...
        no_rels = {
             "emotions": [], 
             "aggregate_vac": {}, "reasoning": "r", 
             "complexity_score": 0.1, "emotional_clarity": 0.1, "temporal_pattern": "c"
        }
        with pytest.raises(Exception): # Validation error on empty emotions or vac
             analyzer._process_llm_response(f"```json{json.dumps(no_rels)}```", "t")

        # Test 3: Missing complexity/clarity
        no_scores = {
             "emotions": [{"emotion_name": "E", "confidence": 0.9, "prominence": "primary", "category": "C", "vac": {"valence": 0, "arousal": 0, "connection": 0}}],
             "aggregate_vac": {"valence": 0, "arousal": 0, "connection": 0},
             "temporal_pattern": "c", "reasoning": "r"
        }
        # Skips 870, 872.
        with pytest.raises(Exception): 
        ...
        no_vac = {
             "emotions": [{"emotion_name": "E", "confidence": 0.9, "prominence": "primary", "category": "C"}],
             "aggregate_vac": {"valence": 0, "arousal": 0, "connection": 0},
             "temporal_pattern": "c", "reasoning": "r", "complexity_score": 0.5, "emotional_clarity": 0.5
        }
    ...
    def test_process_llm_response_relationships_valid(self, analyzer):
        ...
        valid_json = {
             "emotions": [{"emotion_name": "A", "confidence": 0.9, "vac": {"valence": 0, "arousal": 0, "connection": 0}, "category": "C", "prominence": "primary"}, 
                          {"emotion_name": "B", "confidence": 0.9, "vac": {"valence": 0, "arousal": 0, "connection": 0}, "category": "C", "prominence": "secondary"}],
             "relationships": [
                 {"emotion_a": "A", "emotion_b": "B", "type": "contradictory", "strength": 0.8, "description": "desc"},
    ...
    async def test_refresh_prompts(self, analyzer):
        ...
        mock_fetcher.get_prompt_for_function.return_value = {
            "template_content": "New System", "version": 2
        }
        
        with patch("app.services.multi_emotion_analyzer.get_model_fetcher", return_value=mock_fetcher):
            await analyzer._refresh_prompts()
            
            # Check internals (implementation detail, but needed to verify state change)
            # The prompt format requires input_text if validation is strict or just check template source
