...
def mock_ollama_manager():
    with patch('app.api.routes.ai_models.OllamaManager') as mock, \
        ...
        instance.list_local_models = AsyncMock(return_value=[
            {"name": "test-model", "size": 100, "modified_at": "now", "digest": "sha", 
             "parameter_size": "8B", "quantization": "Q4", "family": "llama"}
        ])
        instance.delete_model = AsyncMock(return_value={"status": "success"})
        instance.get_model_details = AsyncMock(return_value={
            "name": "test-model", "size": 100, "parameters": "params", "template": "tpl",
            "format": "gguf", "family": "llama", "parameter_size": "8B",
            "quantization_level": "Q4", "estimated_ram_gb": 8.0, 
            "estimated_speed_tokens_per_sec": 20.0, "recommended_for": []
        })
