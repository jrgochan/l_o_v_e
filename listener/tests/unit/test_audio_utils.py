...
def mock_ffmpeg() -> Any:
    with patch("app.utils.audio_utils.ffmpeg") as mock:
        ...
        class MockFFmpegError(Exception):
            def __init__(self, cmd: Any, stdout: Any, stderr: Any) -> None:
                self.cmd = cmd
                self.stdout = stdout
                self.stderr = stderr
...
class TestAudioProcessor:
    def test_validate_audio_file_exists(self) -> None:
    ...
    def test_get_audio_duration_error(self, mock_ffmpeg: Any) -> None:
        """Test duration error handling."""
        mock_ffmpeg.probe.side_effect = Exception("Probe failed")
        duration = AudioProcessor.get_audio_duration("test.wav")
        assert duration == 0.0

    def test_get_audio_info(self, mock_ffmpeg: Any) -> None:
    ...
    def test_get_audio_info_no_stream(self, mock_ffmpeg: Any) -> None:
    ...
    def test_get_audio_info_error(self, mock_ffmpeg: Any) -> None:
        """Test audio info extraction error."""
        mock_ffmpeg.probe.side_effect = Exception("Probe failed")
        info = AudioProcessor.get_audio_info("test.wav")
        assert not info

    def test_normalize_audio(self, mock_ffmpeg: Any) -> None:
    ...
    def test_normalize_audio_invalid_input(self) -> None:
    ...
    def test_normalize_audio_ffmpeg_error(self, mock_ffmpeg: Any) -> None:
    ...
    def test_convert_to_wav(self, mock_ffmpeg: Any) -> None:
        """Test wav conversion."""
        # Setup chain
        stream_mock = MagicMock()
        mock_ffmpeg.input.return_value = stream_mock
    ...
    def test_convert_to_wav_error(self, mock_ffmpeg: Any) -> None:
    ...
    def test_convert_to_wav_error_with_output_path(self, mock_ffmpeg: Any) -> None:
    ...
    def test_normalize_audio_default_output(self, mock_ffmpeg: Any, tmp_path: Any) -> None:
        ...
        with patch.object(AudioProcessor, "validate_audio_file", return_value=True):
            ...
            expected_output = tmp_path / "test_normalized.wav"

            path = AudioProcessor.normalize_audio(str(input_file), None)
            assert path == str(expected_output)
            mock_ffmpeg.run.assert_called_once()

    def test_normalize_audio_explicit_output(self, mock_ffmpeg: Any, tmp_path: Any) -> None:
        ...
        with patch.object(AudioProcessor, "validate_audio_file", return_value=True):
            ...
            path = AudioProcessor.normalize_audio(str(input_file), str(output_file))
            assert path == str(output_file)
            mock_ffmpeg.output.assert_called_with(
                stream_mock, str(output_file), acodec="pcm_s16le", ac=1, ar=16000, loglevel="error"
            )

    def test_convert_to_wav_default_output(self, mock_ffmpeg: Any, tmp_path: Any) -> None:
        mock_ffmpeg.run.return_value = (b"", b"")
        # Setup ffmpeg chain
        stream_mock = MagicMock()
        mock_ffmpeg.input.return_value = stream_mock
        ...
        input_file = tmp_path / "test.mp3"
        expected_output = tmp_path / "test.wav"

        path = AudioProcessor.convert_to_wav(str(input_file), None)
        assert path == str(expected_output)
        mock_ffmpeg.run.assert_called_once()
...
def test_cleanup_temp_files_missing() -> None:
...
def test_cleanup_temp_files_error() -> None:
