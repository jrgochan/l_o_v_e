...
def mock_model_fetcher():
    with patch('app.services.semantic_analyzer.get_model_fetcher') as mock:
        ...
        fetcher.get_prompt_for_function = AsyncMock(return_value={"template_content": "System msg", "version": "1.0"})
        mock.return_value = fetcher
        yield mock

@pytest.mark.asyncio
async def test_init_defaults(mock_ollama, mock_model_fetcher):
...
@pytest.mark.asyncio
async def test_init_dynamic_fetch(mock_ollama, mock_model_fetcher):
...
@pytest.mark.asyncio
async def test_init_dynamic_fetch_no_loop(mock_ollama, mock_model_fetcher):
