...
def mock_transcription_service():
    with patch("app.services.transcription.get_transcription_service") as mock_get:
        service = Mock()
        result = Mock()
...
def mock_prosody_analyzer():
    with patch("app.services.prosody_analyzer.get_prosody_analyzer") as mock_get:
        analyzer = Mock()
...
def mock_semantic_analyzer():
    with patch("app.services.semantic_analyzer.get_semantic_analyzer") as mock_get:
        analyzer = Mock()
        emotion = AsyncMock()
...
@pytest.fixture
def mock_multi_emotion_analyzer():
    with patch("app.services.multi_emotion_analyzer.get_multi_emotion_analyzer") as mock_get:
        analyzer = Mock()
        
        # Mock analysis result
        mock_analysis = Mock()
        mock_analysis.emotions = [
            Mock(emotion_name="Joy", category="Positive", vac=Mock(valence=0.8, arousal=0.6, connection=0.7), confidence=0.9, prominence="primary"),
            Mock(emotion_name="Excitement", category="High Energy", vac=Mock(valence=0.7, arousal=0.8, connection=0.6), confidence=0.8, prominence="secondary")
        ]
        mock_analysis.relationships = [
            Mock(emotion_a="Joy", emotion_b="Excitement", type="reinforcement", strength=0.8, description="Joy reinforces excitement")
        ]
...
def mock_pii_scrubber():
    with patch("app.services.pii_scrubber.get_pii_scrubber") as mock_get:
        scrubber = Mock()
...
class TestIngestRoutes:
    ...
    def test_extract_audio_features_success(self, mock_transcription_service, mock_prosody_analyzer, mock_deps):
        ...
        with patch("aiofiles.open", return_value=mock_opener) as mock_file_open:
    ...
    def test_analyze_multi_emotion_success_no_prosody(self, mock_multi_emotion_analyzer, mock_pii_scrubber, mock_deps):
    ...
    def test_analyze_multi_emotion_with_invalid_prosody_json(self, mock_multi_emotion_analyzer, mock_pii_scrubber, mock_deps):
    ...
    def test_analyze_multi_emotion_with_prosody_3way(self, mock_multi_emotion_analyzer, mock_pii_scrubber, mock_deps):
    ...
    def test_extract_audio_features_error(self, mock_deps):
    ...
    def test_ingest_audio_upload_success(self, mock_deps):
        ...
        with patch("app.api.routes.ingest.create_pool", new_callable=AsyncMock) as mock_pool_factory, \
             patch("aiofiles.open", return_value=mock_opener) as mock_file_open:
    ...
    def test_get_status_complete(self, mock_deps):
        """Test status endpoint for completed job."""
        with patch("app.api.routes.ingest.create_pool", new_callable=AsyncMock) as mock_pool_factory, \
             patch("app.api.routes.ingest.Job") as MockJob:
