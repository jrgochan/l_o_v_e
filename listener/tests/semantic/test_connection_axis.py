...
class TestConnectionAxis:
    ...
    async def test_pity_vs_compassion(self, pity_text, compassion_text):
        ...
        assert (
            compassion_result.primary_emotion == "Compassion"
        ), f"Expected Compassion, got {compassion_result.primary_emotion}"

        assert compassion_result.vac.connection > 0.5, (
            f"Compassion must have POSITIVE Connection (>0.5), "
            f"got {compassion_result.vac.connection:.2f}"
        )

        assert (
            compassion_result.confidence > 0.7
    ...
    async def test_belonging_vs_fitting_in(self, belonging_text, fitting_in_text):
        ...
        assert (
            belonging_result.vac.connection > 0.5
        ...
        assert (
            fitting_result.vac.connection < 0.0
...
class TestVACExtraction:
    ...
    async def test_vac_values_in_range(self, sample_text):
        ...
        assert -1.0 <= result.vac.valence <= 1.0, f"Valence out of range: {result.vac.valence}"

        assert -1.0 <= result.vac.arousal <= 1.0, f"Arousal out of range: {result.vac.arousal}"

        assert (
            -1.0 <= result.vac.connection <= 1.0
...
#
# 3. Run critical test:
#    pytest tests/semantic/test_connection_axis.py::TestConnectionAxis::test_pity_vs_compassion \
#    -v -s
#
