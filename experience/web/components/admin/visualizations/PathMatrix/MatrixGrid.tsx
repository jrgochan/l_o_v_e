...
export function MatrixGrid({
  ...
  if (viewMode === "categories") {
    ...
    return (
      <div className="inline-block">
        ...
        <div className="flex">
          <div className="w-48 h-12 flex-shrink-0 sticky left-0 bg-gray-900 z-20" />
          {categories.map((category) => (
            <div
              key={category}
              className="w-16 h-12 flex-shrink-0 sticky top-0 bg-gray-900 z-10 flex items-center justify-center"
        ...
        {categories.map((fromCategory) => (
          <div key={fromCategory} className="flex">
            {/* Row Header */}
            <div className="w-48 h-16 flex-shrink-0 sticky left-0 bg-gray-900 z-10 flex items-center px-3 border-r border-gray-700">
              <span className="text-sm text-gray-300 truncate">{fromCategory}</span>
            </div>
            ...
            {categories.map((toCategory) => {
              ...
              return (
                <div
                  key={toCategory}
                  className={`w-16 h-16 flex-shrink-0 border border-gray-800 cursor-pointer transition-all flex items-center justify-center ${
                ...
                >
                  {stats && !isSelf && (
                    <span className="text-[10px] text-white font-mono opacity-70 font-semibold">
  ...
  return (
    <div className="inline-block">
      ...
      <div className="flex">
        <div className="w-32 h-8 flex-shrink-0 sticky left-0 bg-gray-900 z-20" />
        {sortedEmotions.map((emotion) => (
          <div
            key={emotion.id}
            className="w-8 h-8 flex-shrink-0 sticky top-0 bg-gray-900 z-10"
            style={{ writingMode: "vertical-rl" }}
          >
            <span className="text-[10px] text-gray-400 truncate">{emotion.name}</span>
          </div>
        ))}
      </div>
      ...
      {sortedEmotions.map((fromEmotion) => (
        <div key={fromEmotion.id} className="flex">
          ...
          <div className="w-32 h-8 flex-shrink-0 sticky left-0 bg-gray-900 z-10 flex items-center px-2 border-r border-gray-700">
            <span className="text-[10px] text-gray-300 truncate">{fromEmotion.name}</span>
          </div>
          ...
          {sortedEmotions.map((toEmotion) => {
            ...
            return (
              <div
                key={toEmotion.id}
                className={`w-8 h-8 flex-shrink-0 border border-gray-800 cursor-pointer transition-all ${
