...
export function AutoLinkIndicator({ relationships, onRelationshipClick }: AutoLinkIndicatorProps) {
      ...
      {relationships.map((rel) => (
        ...
        >
          ...
          {rel.metadata && typeof rel.metadata.score === "number" && (
            <span className="opacity-75">{((rel.metadata.score as number) * 100).toFixed(0)}%</span>
          )}
        </button>
      ))}
    </div>
