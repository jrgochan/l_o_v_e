      ...
      <section>
        <div className="relative group">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <span className={`${theme.colors.text.muted} group-focus-within:${theme.colors.primary} transition-colors`}>🔍</span>
          </div>
          <input
            type="text"
            value={searchQuery}
        ...
        <section className="animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          <div className="space-y-1">
            {filteredEmotions.length === 0 ? (
              <div className={`text-center py-8 text-sm italic border ${theme.colors.border} ${theme.layout.borderRadius} bg-black/10 ${theme.colors.text.muted}`}>
              ...
              filteredEmotions.map((emotion) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full px-3 py-2.5 text-left text-sm transition-all border ${theme.layout.borderRadius} ${isSelected
