...
export function AIModelsSettings() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* View Tabs */}
      <div className={`flex gap-2 border-b ${theme.colors.border} pb-px`}>
        <button
          onClick={() => setViewMode("models")}
          className={`px-4 py-2 text-sm font-medium transition border-b-2 ${
            viewMode === "models"
              ? `${theme.colors.primary} border-current`
              : `border-transparent ${theme.colors.text.muted} hover:${theme.colors.text.secondary}`
          }`}
        >
      ...
      {!isLoading && viewMode === "models" && (
        <div>
          ...
          {/* Quick Presets */}
          {localModels.length > 0 && (
            <div className={`mb-4 p-4 bg-black/20 border ${theme.colors.border} rounded-lg`}>
              <h4 className={`text-sm font-semibold ${theme.colors.text.primary} mb-3`}>
                ⚡ Quick Presets
              </h4>
              <div className="flex gap-2 flex-wrap">
                {Object.entries(MODEL_PRESETS).map(([key, preset]) => (
                  <button
                    ...
                    className={`px-4 py-2.5 bg-black/20 hover:bg-black/30 border ${theme.colors.border} hover:border-cyan-500/50 rounded transition group`}
                  >
                    <div className="flex items-center gap-2">
                      <span className="text-lg">{preset.icon}</span>
                      <div className="text-left">
                        <div
                          className={`font-medium ${theme.colors.text.primary} text-sm group-hover:${theme.colors.primary} transition`}
                        >
                          {preset.name}
                        </div>
          ...
          {localModels.length > 0 && (
            <div className="mb-4 space-y-3">
              <input
                ...
                className={`w-full px-4 py-2 bg-black/20 border ${theme.colors.border} focus:border-cyan-500 rounded ${theme.colors.text.primary} placeholder-white/20 focus:outline-none transition`}
              />
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs font-medium rounded transition ${
                      familyFilter === null
                        ...
                        : `bg-black/20 ${theme.colors.text.secondary} hover:bg-black/30 border ${theme.colors.border}`
                    }`}
                  >
                    All
                  </button>
          ...
          {localModels.length === 0 ? (
            <div className={`bg-black/20 rounded-lg p-8 border ${theme.colors.border} text-center`}>
              <p className={`${theme.colors.text.secondary} mb-4`}>No models installed yet.</p>
              <button
                onClick={() => setShowPullDialog(true)}
            ...
            <div className={`bg-black/20 rounded-lg p-8 border ${theme.colors.border} text-center`}>
              <p className={`${theme.colors.text.secondary} mb-2`}>No models match your search</p>
              <button
                onClick={() => {
                  setSearchQuery("");
      ...
      {assignDialogState.isOpen && (
        ...
        >
          <div className="bg-gray-900 rounded-lg p-6 max-w-md w-full mx-4 border border-cyan-500/50">
            ...
            <div className={`border-t ${theme.colors.border} pt-3 mb-2`}>
            ...
            <div className="space-y-2 mb-4">
              {functions.map((func) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 bg-black/40 hover:bg-black/60 border ${theme.colors.border} hover:border-cyan-500/50 rounded transition`}
                >
                  <div className={`font-medium ${theme.colors.text.primary} text-sm`}>
            ...
            <button
              ...
              className={`w-full px-4 py-2 bg-black/40 hover:bg-black/60 ${theme.colors.text.primary} rounded transition`}
            >
              Cancel
            </button>
          </div>
