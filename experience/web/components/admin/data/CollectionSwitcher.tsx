...
export function CollectionSwitcher() {
  ...
  const { collections, setCollections, setActiveCollection } = useVisualizationStore();

  const loadCollections = useCallback(async () => {
    setLoading(true);
    try {
      const data = await visualizationService.getCollections();
  ...
  const handleActivate = async (id: string) => {
    setActivating(id);
    try {
      await visualizationService.activateCollection(id);
