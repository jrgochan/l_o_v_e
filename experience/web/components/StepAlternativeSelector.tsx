...
import { therapeuticService, StepAlternative } from "../services/therapeuticService";
...
export const StepAlternativeSelector: React.FC<StepAlternativeSelectorProps> = ({
  ...
  const [alternatives, setAlternatives] = useState<StepAlternative[]>([]);
  ...
  const handleSelect = (alt: StepAlternative) => {
  ...
  return (
    <div className="mt-4 p-4 bg-black/40 rounded-xl border border-white/10 animate-in slide-in-from-top-2">
      ...
      {isLoading && (
        <div className="text-xs text-white/40 text-center py-4">Finding valid paths...</div>
      )}
        ...
        <div className="space-y-2 max-h-60 overflow-y-auto">
          ...
          {alternatives.length === 0 && (
            <div className="text-xs text-white/30 text-center italic">
              No valid alternatives found for this specific transition.
            </div>
          )}
        </div>
      )}
