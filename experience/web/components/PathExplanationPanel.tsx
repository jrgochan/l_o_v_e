...
export const PathExplanationPanel = () => {
  const { primaryPath, selectedStepIndex, showExplanation, toggleExplanation, selectStep } =
  ...
  return (
    <div className="fixed right-0 top-0 h-full w-96 bg-black/80 backdrop-blur-xl border-l border-white/10 p-6 overflow-y-auto z-50 transition-transform duration-300 ease-out">
          ...
          >
            <div className="flex justify-between items-center mb-2">
              ...
              {step.clinical_rationale && (
                <span className="text-[10px] uppercase tracking-wider text-amber-400/80 border border-amber-400/30 px-1.5 py-0.5 rounded">
                  Clinical Note
                </span>
              )}
            </div>
            ...
            <p className="text-white/70 text-sm leading-relaxed mb-3 font-light">{step.summary}</p>
        ...
        <div className="flex justify-between items-end">
          <div>
            ...
            <div className="text-xl font-light text-white/90">
              {primaryPath.path_metrics?.estimated_time || "15-30m"}
            </div>
          </div>
          <div>
            <div className="text-xs text-white/30 uppercase tracking-widest mb-1 text-right">
              Difficulty
            </div>
            <div className="text-xl font-light text-white/90 capitalize text-right">
              {primaryPath.path_metrics?.difficulty || "Moderate"}
            </div>
          </div>
        </div>
      </div>
