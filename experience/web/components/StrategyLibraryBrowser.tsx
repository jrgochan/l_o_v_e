...
export const StrategyLibraryBrowser = () => {
    const { strategies, filters, setFilters, isLoading, setError, fetchStrategies, selectedStrategy, selectStrategy } = useStrategyBrowserStore();
                        ...
                        {strategies.map(strategy => (
                            ...
                            >
                                <div className="flex justify-between items-start mb-4">
                                    <div className="text-xs font-mono text-white/30 uppercase tracking-wider">{strategy.type?.replace(/_/g, ' ')}</div>
                                    <span className={`text-[10px] px-2 py-1 rounded-full border ${getEvidenceColor(strategy.evidence_level)}`}>
                                        {strategy.evidence_level === 'meta_analysis' ? 'META' : strategy.evidence_level?.toUpperCase()}
                                    </span>
                                </div>

                                <h3 className="text-xl font-light text-white mb-2 group-hover:text-cyan-300 transition-colors">{strategy.name}</h3>
                                ...
                                <div className="border-t border-white/5 pt-4 flex justify-between items-center text-xs text-white/40">
                                     <div className="flex items-center gap-1">
                                        ...
                                        <div className="flex">
                                            {[...Array(5)].map((_, i) => (
                                                <div key={i} className={`w-1 h-3 rounded-full mx-0.5 ${i < strategy.difficulty_level ? 'bg-white/40' : 'bg-white/5'}`} />
                                            ))}
                                        </div>
                                     </div>
...
const getEvidenceColor = (level: string) => {
    switch(level) {
        case 'meta_analysis': return 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400';
        case 'rct': return 'bg-cyan-500/10 border-cyan-500/30 text-cyan-400';
        case 'clinical': return 'bg-amber-500/10 border-amber-500/30 text-amber-400';
