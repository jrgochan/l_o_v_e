...
describe("PullModelDialog", () => {
  ...
  it("shows unknown status correctly", async () => {
    const progress = {
      "unknown-model": { status: "unknown", total: 100, completed: 10, percent: 10 },
    };
    render(
      <PullModelDialog
        isOpen={true}
  ...
  it("auto-closes on success", async () => {
    ...
    rerender(
      <PullModelDialog
        ...
        onClose={mockOnClose}
        onPull={mockOnPull}
        pullProgress={{ fast: { status: "success" } } as any}
        localModels={[]}
      />
    );
  ...
  it("handles already installed model flow", async () => {
    ...
    render(
      <PullModelDialog
        ...
        onPull={mockOnPull}
        pullProgress={{}}
        localModels={[{ name: "llama3", size: 0, family: "" } as any]}
      />
    );
  ...
  it("auto-closes on server-side already_installed status", async () => {
    ...
    rerender(
      <PullModelDialog
        ...
        onClose={mockOnClose}
        onPull={mockOnPull}
        pullProgress={{ "installed-model": { status: "already_installed" } } as any}
        localModels={[]}
      />
    );

    await waitFor(() =>
      expect(screen.getAllByText("Already Installed!").length).toBeGreaterThan(0)
    );
  ...
  it("does not auto-close quickly if download took a long time", async () => {
    ...
    rerender(
      <PullModelDialog
        ...
        onClose={mockOnClose}
        onPull={mockOnPull}
        pullProgress={{ "slow-success": { status: "success" } } as any}
        localModels={[]}
      />
    );
