...
describe("CollectionSwitcher", () => {
  ...
  beforeEach(() => {
    ...
    (visualizationService.getCollections as jest.Mock).mockResolvedValue({
      collections: mockCollections,
    });
    (visualizationService.activateCollection as jest.Mock).mockResolvedValue({});
  });
  ...
  it("renders loading state if collections are empty and loading", async () => {
    ...
    (visualizationService.getCollections as jest.Mock).mockReturnValue(collectionsPromise);
  ...
  it("loads collections on mount", async () => {
    ...
    await waitFor(() => {
      expect(visualizationService.getCollections).toHaveBeenCalled();
  ...
  it("handles activation click", async () => {
    ...
    await waitFor(() => {
      expect(visualizationService.activateCollection).toHaveBeenCalledWith("c2");
      expect(visualizationService.getCollections).toHaveBeenCalledTimes(2); // Initial + Refresh
  ...
  it("handles activation error", async () => {
    ...
    (visualizationService.activateCollection as jest.Mock).mockRejectedValue(new Error("Failed"));
  ...
  it("handles error during initial collection load", async () => {
    const consoleSpy = jest.spyOn(console, "error").mockImplementation(() => {});
    (visualizationService.getCollections as jest.Mock).mockRejectedValue(new Error("Load Failed"));
