...
jest.mock("@/components/admin/clinical/VACQuadrantViz", () => ({
    VACQuadrantViz: ({ vac }: any) => (
        <div data-testid="mock-vac-quadrant">V:{vac.valence}</div>
    )
}));
...
describe("ClinicalDashboard", () => {
    ...
    const mockHistory: VACHistoryPoint[] = [
        { timestamp: new Date(), vac: mockVAC, emotion: "Joy" }
    ];
    ...
    describe("Compact View (Normal)", () => {
        ...
        it("renders voice quality indicators correctly", () => {
            const prosodyModerate: ProsodyData = { ...defaultProps.prosody!, voice_quality: "moderate", hnr: 12 };
            const { rerender } = render(<ClinicalDashboard {...defaultProps} prosody={prosodyModerate} />);
            expect(screen.getByText("🟡")).toBeInTheDocument();
        ...
        it("renders status card correctly for different statuses", () => {
            // Critical
            const criticalInsights: InsightData = { clinical_alerts: ["Alert"], overall_status: "critical" };
            const { rerender } = render(<ClinicalDashboard {...defaultProps} insights={criticalInsights} />);
            expect(screen.getByText("🔴 Critical")).toBeInTheDocument();
            expect(screen.getByText("1 alert")).toBeInTheDocument(); // Singular

            // Warning
            const warningInsights: InsightData = { clinical_alerts: ["A1", "A2"], overall_status: "warning" };
    ...
    describe("Expanded View", () => {
        ...
        it("renders multi-emotion table if data present", () => {
            const multiEmotionData = {
                emotions: [
                    { emotion_name: "Joy", confidence: 0.9 } as DetectedEmotion
                ],
                relationships: [],
