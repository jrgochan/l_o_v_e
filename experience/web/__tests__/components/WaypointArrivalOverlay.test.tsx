...
describe("WaypointArrivalOverlay", () => {
    beforeEach(() => {
        ...
        (useExperienceStore as unknown as jest.Mock).mockImplementation((selector: any) => selector ? selector(mockState) : mockState);
    });
    ...
    it("renders Intermediate Waypoint correctly", () => {
        const waypoints = [
            { emotion: "Hope", category: "Happy", reasoning: "Keep going", strategies: [{ name: "Breathe" }] },
        ];
        // Start=0, Waypoint=1
        setMockState({
            isFlying: true,
