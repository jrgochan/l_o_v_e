...
jest.mock("@/components/admin/shared/WaypointDetailModal", () => ({
    WaypointDetailModal: ({ onClose, onNavigate }: any) => (
        <div data-testid="waypoint-modal">
            ...
            <button onClick={() => onNavigate(1)} data-testid="nav-modal">Nav</button>
        </div>
    ),
}));
...
describe("PathDetailsOverlay", () => {
    ...
    beforeEach(() => {
        ...
        (useAtlasAdminStore as unknown as jest.Mock).mockImplementation((selector: any) => selector({
            allEmotions: [{ name: "Joy", category: "positive" }, { name: "Sadness", category: "negative" }],
    ...
    const mockPath = {
        path_id: "p1",
        current_state: { emotion: "Joy", category: "positive", vac: [1, 1, 1], quaternion: [0, 0, 0, 1] },
        goal_state: { emotion: "Sadness", category: "negative", vac: [-1, -1, -1], quaternion: [0, 0, 0, 1] },
        waypoints: [
            { emotion: "Neutral", vac: [0, 0, 0], reasoning: "Bridge" }
        ],
        path_metrics: {
            total_distance: 10,
    ...
    it("should handle unknown category and missing VAC", () => {
        const explicitPath = {
            path_id: "pX",
            current_state: { emotion: "Mystery", category: "unknown", vac: [0, 0, 0], quaternion: [0, 0, 0, 1] },
            goal_state: { emotion: "Sadness", category: "negative", vac: [-1, -1, -1], quaternion: [0, 0, 0, 1] },
