...
describe("aiService", () => {
  ...
  describe("getAssignments", () => {
    ...
    it("should throw and log error on failure", async () => {
      ...
      await expect(aiService.getAssignments()).rejects.toThrow(
        "Failed to fetch assignments: Server Error"
      );
  ...
  describe("assignModel", () => {
    it("should assign model successfully", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        `${API_BASE_URL}/observer/ai/assignments`,
    ...
    it("should throw on failure", async () => {
      ...
      await expect(aiService.assignModel("func1", "model1")).rejects.toThrow(
        "Failed to assign model: Bad Request"
      );
    });
  });
  ...
  describe("getRecommendations", () => {
    it("should return recommendations", async () => {
      const mockData = {
        recommendations: {
          func1: { recommended: ["m1"], not_recommended: ["m2"], reasoning: "r" },
        },
      };
      (global.fetch as jest.Mock).mockResolvedValueOnce({
        ok: true,
  ...
  describe("getPerformance", () => {
    it("should return performance stats", async () => {
      const mockData = {
        performance: {
          m1: { model: "m1", avg_latency_ms: 10, total_invocations: 1, last_used: null },
        },
      };
      (global.fetch as jest.Mock).mockResolvedValueOnce({
        ok: true,
