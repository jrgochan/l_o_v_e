...
const mockEmotions: AtlasEmotion[] = [
    { id: '1', name: 'Joy', category: 'Positive', description: 'Happy', intensity: 0.8, valence: 0.9, arousal: 0.7, coordinates: { x: 0, y: 0, z: 0 } },
    { id: '2', name: 'Sadness', category: 'Negative', description: 'Sad', intensity: 0.6, valence: -0.8, arousal: -0.4, coordinates: { x: 1, y: 1, z: 1 } },
    { id: '3', name: 'Anger', category: 'Negative', description: 'Mad', intensity: 0.9, valence: -0.5, arousal: 0.8, coordinates: { x: -1, y: -1, z: -1 } },
];
...
describe('useAtlasAdminStore', () => {
    ...
    describe('Emotions & Categories', () => {
        it('should set all emotions and initialize category filters', () => {
            const { setAllEmotions, categoryFilters } = useAtlasAdminStore.getState();
        ...
        it('should enable/disable all categories', () => {
            const { setAllEmotions, disableAllCategories, enableAllCategories } = useAtlasAdminStore.getState();
    ...
    describe('Derived State Helpers', () => {
        ...
        it('should get visible emotions respecting filters', () => {
            const { setAllEmotions, toggleCategoryFilter, getVisibleEmotions } = useAtlasAdminStore.getState();
...
describe('Additional Actions & Helpers', () => {
    ...
    it('should handle simple setters', () => {
        ...
        act(() => {
            ...
            store.setPathAnimationMode('flow');
        });
        ...
        expect(state.settings.pathAnimationMode).toBe('flow');
    });
