...
const MOCK_PATHS = new Map([
    ["path1", { id: "path1", from: MOCK_EMOTIONS[0], to: MOCK_EMOTIONS[1], name: "Joy to Sadness" }]
]);
...
describe("useCommandPaletteFilter", () => {
    ...
    it("should filter by category prefix", () => {
        const { result } = renderHook(() => useCommandPaletteFilter({
            search: ">negative",
    ...
    it("should find similar emotions", () => {
        ...
        const EXTENDED_MOCK = [
            ...MOCK_EMOTIONS,
            { id: "5", name: "Bliss", category: "positive", vac: [0.95, 0.95, 0.95], definition: "Pure bliss" }
        ];
        (useAtlasAdminStore as unknown as jest.Mock).mockImplementation((selector) =>
    ...
    it("should filter paths by context (single selection)", () => {
        ...
        expect(result.current.filteredPaths[0].name).toBe("Joy to Sadness");
    });
