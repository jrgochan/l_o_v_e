...
class MockMediaRecorder {
    state = "inactive";
    start = mockStart.mockImplementation(() => { this.state = "recording"; });
    stop = mockStop.mockImplementation(() => {
        this.state = "inactive";
        if (this.onstop) this.onstop();
    });
    pause = mockPause.mockImplementation(() => { this.state = "paused"; });
    resume = mockResume.mockImplementation(() => { this.state = "recording"; });
...
jest.mock("../../../hooks/voice/utils/audioUtils", () => ({
    ...
    processAudioBlob: jest.fn().mockResolvedValue({ blob: new Blob(), url: "mock-url", base64: "base64" }),
...
describe("useMediaRecorder", () => {
    ...
    it("should handle error starting recording", async () => {
        ...
        const { result } = renderHook(() => useMediaRecorder({ state: mockState, actions: mockActions, onError }));
    ...
    it("should handle data availability", async () => {
        const { result } = renderHook(() => useMediaRecorder({ state: mockState, actions: mockActions }));
...
class AdvancedMockMediaRecorder {
    ...
    ondataavailable: Function | null = null;
    onstop: Function | null = null;
    start = jest.fn().mockImplementation(() => { this.state = "recording"; });
    stop = jest.fn().mockImplementation(() => {
        this.state = "inactive";
        if (this.onstop) this.onstop();
    });
    pause = jest.fn().mockImplementation(() => { this.state = "paused"; });
    resume = jest.fn().mockImplementation(() => { this.state = "recording"; });
