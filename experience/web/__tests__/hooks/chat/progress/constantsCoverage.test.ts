import { initializeProgressStages, getAdaptiveMessage } from "../../../../hooks/chat/progress/constants";
...
describe("Progress Constants Coverage", () => {
    ...
    describe("getAdaptiveMessage", () => {
        it("should return correct messages for deepFeeling modes", () => {
            expect(getAdaptiveMessage("emotions", "pending", "warm", true)).toContain("Exploring the layers");
            expect(getAdaptiveMessage("emotions", "pending", "warm", false)).toContain("Identifying your emotional state");
            expect(getAdaptiveMessage("emotions", "pending", "clinical", true)).toContain("Executing multi-emotion");
            expect(getAdaptiveMessage("emotions", "pending", "clinical", false)).toContain("Running semantic");
        });
        ...
        it("should return default message for unknown tone branch (if applicable, though TS prevents it)", () => {
            // Force unknown tone
            expect(getAdaptiveMessage("emotions", "pending", "rudetone" as any, false)).toBe("Processing...");
        });
    });
});
