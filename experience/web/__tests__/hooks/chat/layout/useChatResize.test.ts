...
describe("useChatResize", () => {
    ...
    it("should update height on mouse move (drag up)", () => {
        const { result } = renderHook(() => useChatResize({ isExpanded: true, defaultHeight: 300, minHeight: 100, maxHeight: 600 }));
    ...
    it("should clamp height to min/max", () => {
        const { result } = renderHook(() => useChatResize({ isExpanded: true, defaultHeight: 300, minHeight: 200, maxHeight: 400 }));
