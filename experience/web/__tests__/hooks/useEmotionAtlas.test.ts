...
describe("useEmotionAtlas", () => {
    ...
    it("should fetch emotions on mount if store is empty", async () => {
        const mockResponse = {
            emotions: [
                { id: "e1", name: "Joy", category: "Positive", definition: "", vac: [0.8, 0.8, 0.8], quaternion: {}, color_hint: "" }
            ]
        };
        ...
        expect(logger.info).toHaveBeenCalledWith(expect.anything(), expect.stringContaining("Loaded 1 emotions"));
    });
    ...
    it("should patch suspicious zero VAC with canonical if available", async () => {
        const mockResponse = {
            emotions: [
                { id: "e1", name: "Joy", category: "Positive", definition: "", vac: [0, 0, 0], quaternion: {}, color_hint: "" } // Zero VAC
            ]
        };
        fetchMock.mockResponseOnce(JSON.stringify(mockResponse));
