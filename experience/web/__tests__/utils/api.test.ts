...
describe("Admin API Helpers", () => {
  ...
  it("fetches sessions", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining("/admin/sessions?skip=0&limit=10"),
      expect.anything()
    );
  });
  ...
  it("updateStrategy", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining("/admin/strategies/1"),
      expect.objectContaining({ method: "PUT" })
    );
  });
  ...
  it("updateBootstrapData", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining("/admin/bootstrap/1"),
      expect.objectContaining({ method: "PUT" })
    );
  });
  ...
  it("getPromptTemplates", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining("function_name=test-func"),
      expect.anything()
    );
  });
  ...
  it("updatePromptTemplate", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining("/admin/prompts/1"),
      expect.objectContaining({ method: "PUT" })
    );
  });
  ...
  it("updateAiModel", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining("/admin/ai-models/func"),
      expect.objectContaining({ method: "PUT" })
    );
  });
  ...
  it("getSessionDetails", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining("/admin/sessions/1"),
      expect.anything()
    );
  });
