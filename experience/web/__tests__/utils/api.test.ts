...
describe('API Utils', () => {
    ...
    describe('Admin API Helpers', () => {
        ...
        it('should call getSessionDetails with correct ID', async () => {
            ...
            expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('/admin/sessions/123'), expect.anything());
        });

        it('should call getAtlasEmotions', async () => {
            fetchMock.mockResponseOnce(JSON.stringify([]));
            await adminApi.getAtlasEmotions();
            expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('/admin/atlas/emotions'), expect.anything());
        });

        it('should call updateAtlasEmotion', async () => {
            fetchMock.mockResponseOnce(JSON.stringify({}));
            await adminApi.updateAtlasEmotion('e1', { description: 'test' });
            expect(fetchMock).toHaveBeenCalledWith(
        ...
        it('should call exportAtlasData', async () => {
            ...
            expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('/admin/atlas/export'), expect.anything());
        });
        ...
        it('should call getAiModels', async () => {
            ...
            expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('/admin/ai-models'), expect.anything());
        });

        it('should call updateAiModel', async () => {
            fetchMock.mockResponseOnce(JSON.stringify({}));
            await adminApi.updateAiModel('chat', { model: 'gpt' });
            expect(fetchMock).toHaveBeenCalledWith(
        ...
        it('should call getStrategies', async () => {
            ...
            expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('/admin/strategies'), expect.anything());
        });

        it('should call updateStrategy', async () => {
            fetchMock.mockResponseOnce(JSON.stringify({}));
            await adminApi.updateStrategy('s1', { name: 'New' });
            expect(fetchMock).toHaveBeenCalledWith(
        ...
        it('should call exportStrategies', async () => {
            ...
            expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('/admin/strategies/export'), expect.anything());
        });
        ...
        it('should call createBootstrapData', async () => {
            fetchMock.mockResponseOnce(JSON.stringify({}));
            await adminApi.createBootstrapData({ type: 'test', data: {} });
            expect(fetchMock).toHaveBeenCalledWith(
        ...
        it('should call updateBootstrapData', async () => {
            fetchMock.mockResponseOnce(JSON.stringify({}));
            await adminApi.updateBootstrapData('b1', { data: { foo: 'bar' } });
            expect(fetchMock).toHaveBeenCalledWith(
        ...
        it('should call getPromptTemplates', async () => {
            ...
            expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('/admin/prompts'), expect.anything());

            fetchMock.mockResponseOnce(JSON.stringify([]));
            await adminApi.getPromptTemplates('chat');
            expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('function_name=chat'), expect.anything());
        });
        ...
        it('should call createPromptTemplate', async () => {
            ...
            expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('/admin/prompts'), expect.objectContaining({ method: 'POST' }));
        });

        it('should call updatePromptTemplate', async () => {
            fetchMock.mockResponseOnce(JSON.stringify({}));
            await adminApi.updatePromptTemplate('p1', { content: 'updated' });
            expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('/admin/prompts/p1'), expect.objectContaining({ method: 'PUT' }));
        });
        ...
        it('should call testPromptTemplate', async () => {
            ...
            expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('/admin/prompts/test'), expect.objectContaining({ method: 'POST' }));
        });
    });
});
