...
jest.mock("@/components/CinematicOverlay", () => ({
    CinematicOverlay: ({ onEnableAudio }: any) => (
        <div data-testid="cinematic-overlay">
            <button data-testid="enable-audio-btn" onClick={onEnableAudio}>Enable</button>
        </div>
    ),
}));
...
jest.mock("@/stores/useExperienceStore", () => ({
    useExperienceStore: jest.fn((selector) => selector({
        currentVAC: [0, 0, 0],
        targetVAC: [0, 0, 0],
    })),
}));
...
jest.mock("@/stores/useAtlasAdminStore", () => ({
    useAtlasAdminStore: jest.fn((selector) => selector({
