...
*   **Path Animation**:
    ...
    *   *Dynamic*: Fast, energetic.
    *   *Cinematic*: "Director Mode" (Camera follows the path spline).
    *   *Liquid*: Current default.
*   **Visibility Layers**:
    *   [x] Soul Sphere
    *   [x] Emotion Points
    *   [x] Labels
...
*   **Content**:
    *   *Idle*: Hidden / Minimal status.
    *   *Hover*: Emotion Name + VAC stats.
    *   *Path Active*: "Journey: Anger -> Joy", Step count, "Play/Stop" toggle.
*   **Styling**: Pure SwiftUI `.background(.ultraThinMaterial)` with rounded corners.

## 5. Implementation Plan

### Phase 12: Visualization Core (The Engine)
1.  **Camera Logic**: Update `SoulRenderer` to support Orbit Controls *and* Spline Flyover.
