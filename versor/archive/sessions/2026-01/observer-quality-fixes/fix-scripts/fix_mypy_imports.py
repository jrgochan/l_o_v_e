...
def ensure_typing_imports(filepath: Path) -> int:
    ...
    needs_dict = (
        "Dict[" in content or ": Dict" in content or "-> Dict" in content
    ) and "from typing import" in content
    needs_list = (
        "List[" in content or ": List" in content or "-> List" in content
    ) and "from typing import" in content
    ...
    if typing_import_match:
        ...
        if new_imports:
            ...
            content = content.replace(
                f"from typing import {existing_imports}", f"from typing import {updated_imports}"
            )
