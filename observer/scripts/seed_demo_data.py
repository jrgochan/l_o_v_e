...
async def check_existing_demo_data(session) -> Dict[str, int]:
    ...
    stmt = select(func.count(UserJourney.id))  # pylint: disable=not-callable
    result = await session.execute(stmt)
    ...
    stmt = select(func.count(JourneyWaypoint.id))  # pylint: disable=not-callable
    result = await session.execute(stmt)
    waypoint_count = result.scalar()

    # Count strategy attempts
    stmt = select(func.count(StrategyAttempt.id))  # pylint: disable=not-callable
    result = await session.execute(stmt)
...
async def seed_journey(
    session, journey_data: Dict[str, Any], dry_run: bool = False
) -> tuple[bool, str]:
    """
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print(
            f"Processing Journey: {journey_data['start_emotion']} → {journey_data['goal_emotion']}"
        )
        ...
        if dry_run:
            print(
                f"  🔍 DRY RUN: Would create journey with {len(journey_data['waypoints'])} waypoints"
            )
            print(
                f"  🔍 DRY RUN: Would create {len(journey_data['strategies_attempted'])} strategy attempts"
            )
        ...
        for waypoint_data in journey_data["waypoints"]:
            ...
            if not emotion_id:
                print(
                    f"  ⚠️  WARNING: Could not find emotion ID for {waypoint_data['emotion']}, skipping waypoint"
                )
                continue
        ...
        for attempt_data in journey_data["strategies_attempted"]:
            ...
            if not strategy_id:
                print(
                    f"  ⚠️  WARNING: Could not find strategy ID for '{attempt_data['strategy_name']}', skipping attempt"
                )
                continue
