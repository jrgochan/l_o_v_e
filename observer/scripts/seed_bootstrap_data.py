...
async def seed_bootstrap_patterns(
    session, data: Dict[str, Any], dry_run: bool = False
) -> tuple[int, int]:
    """
...
async def verify_bootstrap_data(session) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"total_records": total, "versions": versions}
...
async def main(dry_run: bool = False, verify_only: bool = False):
    ...
    async with AsyncSessionLocal() as session:
        ...
        print(
            f"  Context modifiers: {len([k for k in data['context_modifiers'].keys() if k not in ['description', 'note']])}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--verify-only", action="store_true", help="Only verify existing bootstrap data"
    )
