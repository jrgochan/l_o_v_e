...
async def seed_category_transitions(
    ...
    if dataset == "brene_brown":
        ...
        print(
            f"  ⚠️  Unknown dataset '{dataset}' for transitions, defaulting to brene_brown structure if available"
        )
    ...
    async with AsyncSessionLocal() as session:
        ...
        if len(existing_mappings) > 0:
            if force_reseed:
                ...
                print(
                    f"  ⏭️  Pattern-strategy mappings already seeded ({len(existing_mappings)} entries)"
                )
                return
        ...
        await session.commit()
        print("✅ Seeded pattern-strategy mappings")
