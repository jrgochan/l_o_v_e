...
def print_step(step_num: int, total_steps: int, message: str):
    ...
    print(
        f"{Colors.OKCYAN}[{step_num}/{total_steps}]{Colors.ENDC} {Colors.BOLD}{message}{Colors.ENDC}"
    )
...
def run_seed_script(
    script_name: str, args: List[str] = None, required: bool = True
) -> Tuple[bool, str]:
    """
...
def main(
    ...
    if dataset.lower() == "all":
        datasets_to_seed = [
            ...
            (
                "GoEmotions (28)",
                "data/goemotions/emotions.json",
                "GoEmotions",
                "--default",
            ),
            (
                "UAL (Unified Affective Lexicon)",
                "data/ual/emotions.json",
                "Unified Affective Lexicon",
            ),
        ]
    ...
    if failed_steps:
        ...
        print(
            f"{Colors.FAIL}{Colors.BOLD}SEEDING INCOMPLETE - Please review errors above{Colors.ENDC}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--with-demo", action="store_true", help="Include demo journey data (DEV ONLY)"
    )
    parser.add_argument(
        "--with-bootstrap",
        action="store_true",
