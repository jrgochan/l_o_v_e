...
def upgrade() -> None:
    ...
    # Create indexes
    op.execute(
        "CREATE INDEX idx_clinical_alerts_session "
        "ON clinical_alerts(session_id)"
    )
    
    op.execute(
        "CREATE INDEX idx_clinical_alerts_timestamp "
        "ON clinical_alerts(timestamp)"
    )
    
    op.execute(
        "CREATE INDEX idx_clinical_alerts_level "
        "ON clinical_alerts(level)"
    )
    
    op.execute(
        "CREATE INDEX idx_clinical_alerts_type "
        "ON clinical_alerts(type)"
    )
