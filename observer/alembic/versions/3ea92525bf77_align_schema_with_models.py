...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE journey_waypoints ALTER COLUMN vac_target TYPE JSONB USING to_jsonb(vac_target)"
    )

    # 3. Fix quaternion_target (FLOAT[4] -> JSONB)
    op.execute(
        "ALTER TABLE journey_waypoints ALTER COLUMN quaternion_target TYPE JSONB USING to_jsonb(quaternion_target)"
    )

    # 4. Fix validated_vac (FLOAT[3] -> JSONB)
    op.execute(
        "ALTER TABLE journey_waypoints ALTER COLUMN validated_vac TYPE JSONB USING to_jsonb(validated_vac)"
    )
