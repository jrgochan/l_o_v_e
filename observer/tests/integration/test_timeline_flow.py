...
@pytest.mark.asyncio
async def test_full_timeline_flow(test_db):  # pylint: disable=too-many-locals
    """
    ...
    context_a = MessageCreationContext(
        session_id=session_id,
        content="I am feeling anxious today.",
        message_type="user_text",
    )
    msg_a = await chat_service.save_user_message(context_a)
    assert msg_a.id
    ...
    # We verify the atomic creation of relationship
    print(f"DEBUG: Creating linked message pointing to {msg_a.id}")
    context_b = MessageCreationContext(
        session_id=session_id,
    ...
    msg_b = await chat_service.save_user_message(context_b)
    assert msg_b.id
