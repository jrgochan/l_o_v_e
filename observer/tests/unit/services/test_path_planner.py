...
from app.services.planning.core import PathPlanner, TransitionPath
...
def mock_emotions():
    def create(name, category, vac):
        return EmotionDefinition(
            id=uuid4(), emotion_name=name, category=category, vac_vector=vac, definition="Def"
        )
...
async def test_find_transition_path_direct(planner, mock_emotions):
    ...
    path = await planner.find_transition_path([0, 0, 0], [0.1, 0, 0])
...
async def test_find_alternative_paths(planner, mock_emotions):
    ...
    paths = await planner.find_alternative_paths([0, 0, 0], [1, 0, 0])
...
async def test_fallback_path(planner, mock_emotions):
    ...
    path = await planner.find_transition_path([0, 0, 0], [1, 1, 1])
...
async def test_find_alternative_paths_error(planner, mock_emotions):
    ...
    with pytest.raises(ValueError, match="Invalid VAC"):
        await planner.find_alternative_paths([0, 0, 0], [1, 1, 1])
...
async def test_find_transition_path_with_collection(planner, mock_emotions):
    ...
    await planner.find_transition_path([0, 0, 0], [1, 0, 0], collection_id="coll_123")
...
async def test_find_alternative_paths_direct(planner, mock_emotions):
    ...
    paths = await planner.find_alternative_paths([0, 0, 0], [0.1, 0, 0])
    assert len(paths) == 1
...
async def test_fallback_path_loop_break(planner, mock_emotions):
    ...
    planner.graph.get_valid_neighbors.side_effect = [[n1], []]  # 1st iter  # 2nd iter (break 162)
    planner.graph.vac_distance.return_value = 0.5
    planner.harmonizer.validate_and_enhance_path.side_effect = lambda p, s, g: p
...
async def test_build_path_metrics_easy_2wp(planner, mock_emotions):
    ...
    path = await planner.find_transition_path([0, 0, 0], [0.3, 0, 0])
...
async def test_user_history_parsing(planner, mock_session, mock_emotions):
    ...
    await planner.find_transition_path([0, 0, 0], [1, 0, 0], user_id=user_id)

    args, _ = planner.searcher.find_path.call_args
    user_hist_arg = args[3]  # 4th arg
...
async def test_user_history_branches(planner, mock_session):
    ...
    j2 = MagicMock()
    j2.waypoints = {
        "waypoints": [{"emotion": "A"}, {"no_emotion": "B"}]  # Missing "emotion" key for to_e
    }
    ...
    # Trigger _get_user_history via find_transition_path
    ...
    hist = await planner._get_user_history("user")
