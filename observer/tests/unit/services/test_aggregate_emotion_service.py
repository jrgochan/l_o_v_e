import pytest
import numpy as np
from app.services.aggregate_emotion_service import AggregateEmotionService
...
def test_determine_temporal_pattern_emerging(service):
    ...
    emotions = [
        # Strong primary
        {"emotion_name": "Joy", "confidence": 0.9, "vac": {"valence": 0.8}, "prominence": "primary"},
        # Weak secondary, very different
        {"emotion_name": "Sadness", "confidence": 0.5, "vac": {"valence": -0.5}, "prominence": "secondary"}
    ]
...
def test_calculate_weighted_vac_zero_confidence(service):
    ...
    emotions = [
        {"emotion_name": "A", "vac": {"valence": 1.0, "arousal": 0.0, "connection":0.0}, "confidence": 0},
        {"emotion_name": "B", "vac": {"valence": -1.0, "arousal": 0.0, "connection":0.0}, "confidence": 0},
    ]
...
def test_private_methods_empty_input(service):
    ...
    assert service._calculate_weighted_vac([]) == {"valence": 0.0, "arousal": 0.0, "connection": 0.0}
...
def test_determine_temporal_pattern_concurrent_explicit(service):
    ...
    emotions = [
        {"emotion_name": "Peace", "confidence": 0.8, "vac": {"valence": 0.1, "arousal": 0, "connection": 0}},
        {"emotion_name": "Joy", "confidence": 0.75, "vac": {"valence": 0.2, "arousal": 0, "connection": 0}}
    ]
