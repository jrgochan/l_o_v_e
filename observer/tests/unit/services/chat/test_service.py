...
async def test_save_user_message(chat_service):
    ...
    context = MessageCreationContext(session_id=sid, content="Hi", message_type="user_text")
    res = await chat_service.save_user_message(context)
    assert res == "Message"
    chat_service.message_manager.save_user_message.assert_called_with(context)
...
async def test_save_analysis_message(chat_service):
    ...
    from app.services.chat.types import AnalysisMessageContext
    ...
    res = await chat_service.save_analysis_message(context)
    assert res == "Analysis"
    chat_service.analysis_manager.save_analysis_message.assert_called_with(context)
...
async def test_save_multi_emotion_analysis(chat_service):
    ...
    from app.services.chat.types import MultiEmotionAnalysisContext
    ...
    res = await chat_service.save_multi_emotion_analysis(context)
    assert res == "Multi"
