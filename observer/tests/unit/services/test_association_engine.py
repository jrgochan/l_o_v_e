...
from app.services.association_engine import AssociationEngine
...
async def test_auto_link_no_matches(mock_session, mock_embedding_service):
    ...
    mock_session.execute.side_effect = [mock_result_msg, mock_result_user, mock_result_similar]
...
async def test_auto_link_missing_user_id(mock_session, mock_embedding_service):
    ...
    msg = ChatMessage(id=uuid4(), session_id=uuid4(), content="No User", semantic_embedding=[0.1]*384)
