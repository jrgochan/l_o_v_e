...
def mock_session():
    ...
    mock_result.scalar_one.side_effect = lambda: joy # Default storage, test will override scalar_one if needed but find_nearest re-queries
    # Actually logic:
...
async def test_find_nearest_no_emotions(emotion_mapper, mock_session):
    ...
    with pytest.raises(ValueError, match="No emotions found"):
        await emotion_mapper.find_nearest([0.0, 0.0, 0.0])
...
async def test_find_nearest_logic(emotion_mapper, mock_session):
    ...
    result = await emotion_mapper.find_nearest([0.8, 0.8, 0.5])
...
async def test_get_top_k_nearest(emotion_mapper, mock_session):
    ...
    results = await emotion_mapper.get_top_k_nearest([0.0, 0.0, 0.0], k=3)
...
async def test_get_top_k_nearest_empty(emotion_mapper, mock_session):
    ...
    with pytest.raises(ValueError, match="No emotions found"):
        await emotion_mapper.get_top_k_nearest([0.5, 0.5, 0.5])
...
async def test_get_top_k_nearest_malformed_embedding(emotion_mapper, mock_session):
    ...
    results = await emotion_mapper.get_top_k_nearest([0.1, 0.2, 0.3], text_embedding=[0.1]*384)
...
async def test_get_top_k_nearest_combinations(emotion_mapper, mock_session):
    ...
    mock_res = MagicMock()
    mock_res.scalars.return_value.all.return_value = [emotion]
    mock_session.execute.side_effect = None
    mock_session.execute.return_value = mock_res
    
    # Case 2: Text embedding provided, word count provided (weighted fusion)
    await emotion_mapper.get_top_k_nearest([0.8, 0.8, 0.8], text_embedding=[0.1]*3, word_count=5)
    
    # Case 3: Text embedding None
    await emotion_mapper.get_top_k_nearest([0.8, 0.8, 0.8], text_embedding=None, word_count=5)
...
async def test_find_nearest_full_fusion(emotion_mapper, mock_session):
    ...
    def side_effect(stmt):
        ...
        if "WHERE" in str(stmt) or "where" in str(stmt):
            ...
            res.scalars.return_value.all.return_value = emotions
        return res
