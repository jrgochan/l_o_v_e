...
from app.services.ai_model_service import AIModelService, get_ai_model_service
...
async def test_assign_model_update(service, mock_db):
    ...
    existing = MagicMock()
    existing.assigned_at = datetime.utcnow() # Needs a value for return
    
    mock_result = MagicMock()
    mock_result.scalar_one_or_none.return_value = existing
    mock_db.execute.return_value = mock_result
    
    res = await service.assign_model("semantic_vac", "updated-model")
...
async def test_get_performance_stats(service, mock_db):
    ...
    assign.last_used_at = datetime.utcnow()
...
async def test_dependency_helper():
    ...
    with patch("app.services.ai_model_service.get_db") as mock_get_db:
