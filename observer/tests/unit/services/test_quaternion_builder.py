...
def builder_direct():
    ...
    with patch("app.services.quaternion_builder.settings"):
...
async def test_from_vac_http_success(builder_http):
    ...
    mock_response_data = {"current_state": {"w": 0.7, "x": 0.7, "y": 0.0, "z": 0.0}}
...
def test_singleton_pattern():
    ...
    from app.services.quaternion_builder import get_quaternion_builder
...
async def test_from_vac_direct_conversion():
    ...
    with patch.dict(
        "sys.modules", {"versor.app.core.vac_model": MagicMock(VACVector=mock_vac_vector_class)}
    ):
        qb = QuaternionBuilder(use_http=False)
