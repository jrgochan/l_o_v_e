...
from app.services.clinical.alerts import ClinicalAlertService
...
async def test_voice_quality_alert(service):
    ...
    context = AnalysisContext(
        vac_data={},
        prosody_data={"hnr": 4.0},
        confidence=0.9,
    )
    alerts = await service.evaluate_alerts(session_id="sess_1", context=context)
    assert len(alerts) == 1
...
async def test_vocal_stability_jitter(service):
    ...
    # Warning (>5.0)
    # Warning (>5.0)
    context_warn = AnalysisContext(vac_data={}, prosody_data={"jitter": 6.0}, confidence=0.9)
    alerts = await service.evaluate_alerts(session_id="sess_1", context=context_warn)
    assert len(alerts) == 1
    assert alerts[0].level == AlertLevel.WARNING.value

    # Attention (>3.0)
    context_attn = AnalysisContext(vac_data={}, prosody_data={"jitter": 4.0}, confidence=0.9)
    alerts2 = await service.evaluate_alerts(session_id="sess_1", context=context_attn)
...
async def test_vocal_stability_shimmer(service):
    ...
    # Warning (>10.0)
    # Warning (>10.0)
    context_warn = AnalysisContext(vac_data={}, prosody_data={"shimmer": 11.0}, confidence=0.9)
    alerts = await service.evaluate_alerts(session_id="sess_1", context=context_warn)
    assert len(alerts) == 1
    assert alerts[0].level == AlertLevel.WARNING.value

    # Attention (>6.0) - NEW
    context_attn = AnalysisContext(vac_data={}, prosody_data={"shimmer": 8.0}, confidence=0.9)
    alerts2 = await service.evaluate_alerts(session_id="sess_1", context=context_attn)
    assert len(alerts2) == 1
    assert alerts2[0].level == AlertLevel.ATTENTION.value
...
async def test_pitch_range_alert(service):
    ...
    # Very narrow (<30)
    # Very narrow (<30)
    context_warn = AnalysisContext(vac_data={}, prosody_data={"pitch_range": 20.0}, confidence=0.9)
    alerts = await service.evaluate_alerts(session_id="sess_1", context=context_warn)
    assert len(alerts) == 1
    assert alerts[0].level == AlertLevel.WARNING.value

    # Narrow (<50) - NEW
    context_attn = AnalysisContext(vac_data={}, prosody_data={"pitch_range": 40.0}, confidence=0.9)
    alerts2 = await service.evaluate_alerts(session_id="sess_1", context=context_attn)
    assert len(alerts2) == 1
    assert alerts2[0].level == AlertLevel.ATTENTION.value
...
async def test_low_confidence(service):
    ...
    # Very Low (<0.4)
    # Very Low (<0.4)
    context_warn = AnalysisContext(vac_data={}, prosody_data=None, confidence=0.3)
    alerts = await service.evaluate_alerts(session_id="sess_1", context=context_warn)
    assert len(alerts) == 1
    ...
    # Low (<0.6) - NEW
    context_attn = AnalysisContext(vac_data={}, prosody_data=None, confidence=0.5)
    alerts2 = await service.evaluate_alerts(session_id="sess_1", context=context_attn)
    assert len(alerts2) == 1
    assert alerts2[0].level == AlertLevel.ATTENTION.value
