...
@pytest.fixture
def mock_emotions():
    e1 = EmotionDefinition(
        id=uuid4(), emotion_name="Joy", category="Cat1", vac_vector=[0.8, 0.6, 0.7]
    )
    e2 = EmotionDefinition(
        id=uuid4(),
        emotion_name="Sadness",
        category="Cat2",
        vac_vector=[-0.8, -0.2, -0.5],
    )
    return [e1, e2]
...
async def test_vac_match_parsing_and_success():
    ...
    row = (uuid4(), "Excitement", "High Energy", "[0.9, 0.8, 0.7]", 0.1)
...
async def test_resolve_emotion_none(resolver, mock_session, mock_emotions):
    ...
    res = await resolver.resolve_emotion(
        "CompletelyRandomString", vac={"valence": 0, "arousal": 0, "connection": 0}
    )
...
async def test_ensure_loaded_with_explicit_collection_id(mock_session):
    ...
    cid = str(uuid4())
    config = ResolverConfig(collection_id=cid, collection_name="Custom")
    resolver = EmotionResolver(mock_session, config=config)
