...
def test_detected_emotion_initialization_and_properties():
    ...
    de = DetectedEmotion(id=uuid4(), confidence=0.9, prominence="primary", vac=[0.5, 0.5, 0.5])
...
def test_detected_emotion_to_dict_full():
    ...
    # Optional fields
    assert data["voice_interpretation_vac"] == {
        "valence": 0.4,
        "arousal": 0.5,
        "connection": 0.6,
    }
...
def test_emotion_goal_model():
    ...
    goal = EmotionGoal(status="active", goal_emotion=EmotionDefinition(emotion_name="Hope"))
...
def test_missing_coverage_branches():
    ...
    er = EmotionRelationship(relationship_type="masking", strength=0.5)
...
def test_emotion_goal_properties():
    ...
    # Active
    g1 = EmotionGoal(session_id=uuid4(), user_id=uuid4(), status="active")
    assert g1.is_active is True
    assert g1.is_achieved is False
    assert g1.is_abandoned is False

    # Achieved
    g2 = EmotionGoal(session_id=uuid4(), user_id=uuid4(), status="achieved")
    assert g2.is_active is False
    assert g2.is_achieved is True

    # Abandoned
    g3 = EmotionGoal(session_id=uuid4(), user_id=uuid4(), status="abandoned")
