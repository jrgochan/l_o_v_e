...
from app.api.schemas.transition import TransitionPathRequest, JourneyStartRequest, WaypointReachedRequest
...
async def test_mark_waypoint_reached_incomplete():
    ...
    mock_wp1 = MagicMock(); mock_wp1.reached = True
    mock_wp2 = MagicMock(); mock_wp2.reached = False
    call_tracker = {"count": 0}
    async def execute_side_effect(stmt, *args, **kwargs):
        call_tracker["count"] += 1
        count = call_tracker["count"]
        if count == 1:
             m = MagicMock(); m.scalar_one_or_none.return_value = mock_journey; return m
        if count == 2:
            m = MagicMock(); m.scalar_one_or_none.return_value = mock_waypoint; return m
        if count == 3:
            m = MagicMock(); mock_scalars = MagicMock(); mock_scalars.all.return_value = [mock_wp1, mock_wp2]; m.scalars.return_value = mock_scalars; return m
        return MagicMock()
    mock_db.execute.side_effect = execute_side_effect
    request = MagicMock(); request.waypoint_index = 0
...
async def test_mark_waypoint_journey_completion():
    ...
    resp = await mark_waypoint_reached(mock_journey.id, request, db=mock_db) # Changed transitions.mark_waypoint_reached to mark_waypoint_reached
...
async def test_journey_status_no_started_at():
    ...
    resp = await get_journey_status(uuid4(), db=mock_db) # Changed transitions.get_journey_status to get_journey_status
...
async def test_journey_status_exception():
    ...
    with pytest.raises(Exception):
        await get_journey_status(uuid4(), db=mock_db) # Changed transitions.get_journey_status to get_journey_status
...
async def test_journey_history_exception():
    ...
    with pytest.raises(Exception):
        await get_user_journey_history(uuid4(), db=mock_db) # Changed transitions.get_user_journey_history to get_user_journey_history
...
async def test_effective_strategies_exception():
    ...
    with pytest.raises(Exception):
        await get_user_effective_strategies(uuid4(), db=mock_db) # Changed transitions.get_user_effective_strategies to get_user_effective_strategies
...
async def test_start_journey(mock_db):
    ...
    with patch("app.api.routes.transitions.UserJourney") as MockUserJourney:
        mock_journey_instance = MagicMock()
        mock_journey_instance.id = str(uuid4()) # Fixed: str
        mock_journey_instance.status = "in_progress"
        mock_journey_instance.current_waypoint = 0
        mock_journey_instance.started_at = datetime.now()
...
async def test_get_journey_status(mock_db):
    ...
    mock_wps = [
        JourneyWaypoint(reached=True),
        JourneyWaypoint(reached=False)
    ]
...
async def test_mark_waypoint_reached_incomplete():
    ...
    # We need to set side_effect but the query order might differ from the simple 1-2-3 logic in the original test
...
def test_estimate_helpers():
    ...
    from app.api.routes.transitions import _estimate_waypoint_time, _estimate_difficulty
    # Time
