...
async def test_chat_websocket_auth_success(mock_websocket, mock_auth_session_local, mock_user):
    ...
    with patch("app.api.sockets.router.manager.connect", new_callable=AsyncMock) as mock_connect:
...
async def test_chat_websocket_ping_pong(mock_websocket, mock_user):
    ...
    with (
        patch("app.api.sockets.router.manager.send_message", new_callable=AsyncMock) as mock_send,
        patch("app.api.sockets.router.manager.connect", new_callable=AsyncMock),
    ):
...
async def test_process_text_message_flow(
    mock_websocket, mock_auth_session_local, mock_chat_service, mock_httpx_client
):
    ...
    mock_httpx_client.post.return_value = MagicMock(
        status_code=200, json=lambda: {"emotion": "Joy"}
    )

    with (
        patch("app.api.sockets.handlers.AsyncSessionLocal", new=mock_auth_session_local),
        patch("app.api.sockets.handlers.ChatService", return_value=mock_chat_service),
        patch("app.api.sockets.handlers.httpx.AsyncClient", return_value=mock_httpx_client),
        patch("app.api.sockets.handlers.manager.get_db_session", return_value=uuid4()),
        patch("app.api.sockets.handlers.manager.send_message", new_callable=AsyncMock),
        patch(
            "app.api.sockets.handlers.handle_single_emotion_result", new_callable=AsyncMock
        ) as mock_handle_single,
    ):
...
@pytest.mark.asyncio
async def test_handle_single_emotion_result_logic(
    mock_websocket, mock_auth_session_local, mock_chat_service
):
...
async def test_handle_tone_update(mock_websocket, mock_auth_session_local, mock_chat_service):
    ...
    with (
        patch("app.api.sockets.handlers.AsyncSessionLocal", new=mock_auth_session_local),
        patch("app.api.sockets.handlers.ChatService", return_value=mock_chat_service),
        patch("app.api.sockets.handlers.manager.get_db_session", return_value=db_session_id),
        patch("app.api.sockets.handlers.manager.send_message", new_callable=AsyncMock) as mock_send,
    ):
...
@pytest.mark.asyncio
async def test_handle_deep_feeling_update(
    mock_websocket, mock_auth_session_local, mock_chat_service
):
...
@pytest.mark.asyncio
async def test_process_audio_message_flow(
    mock_websocket, mock_auth_session_local, mock_chat_service, mock_httpx_client
):
    ...
    with (
        ...
        await process_audio_message("sess_audio", dummy_audio, None, "warm", False, mock_websocket)
...
async def test_handle_user_message_dispatch(mock_websocket):
    ...
    with (
        patch(
            "app.api.sockets.handlers.process_audio_message", new_callable=AsyncMock
        ) as mock_audio,
        patch("app.api.sockets.handlers.manager.send_message", new_callable=AsyncMock),
    ):  # ACK
    ...
    with (
        patch("app.api.sockets.handlers.process_text_message", new_callable=AsyncMock) as mock_text,
        patch("app.api.sockets.handlers.manager.send_message", new_callable=AsyncMock),
    ):
...
@pytest.mark.asyncio
async def test_handle_multi_emotion_result(
    mock_websocket, mock_auth_session_local, mock_chat_service
):
    ...
    with (
        ...
        patch("app.api.sockets.handlers.ChatService", return_value=mock_chat_service),
        patch("app.api.sockets.handlers.manager.send_message", new_callable=AsyncMock),
        patch("app.api.sockets.handlers.send_progress", new_callable=AsyncMock),
        patch(
            "app.api.sockets.handlers.generate_insights", new_callable=AsyncMock
        ) as mock_insights,
    ):
...
async def test_process_text_message_new_session(
    mock_websocket, mock_auth_session_local, mock_chat_service, mock_httpx_client
):
    ...
    with (
        ...
        patch("app.api.sockets.handlers.ChatService", return_value=mock_chat_service),
        patch("app.api.sockets.handlers.httpx.AsyncClient", return_value=mock_httpx_client),
        patch("app.api.sockets.handlers.manager.get_db_session", return_value=None),
        patch("app.api.sockets.handlers.manager.set_db_session") as mock_set_db,
        patch("app.api.sockets.handlers.manager.send_message", new_callable=AsyncMock),
        patch("app.api.sockets.handlers.handle_single_emotion_result", new_callable=AsyncMock),
    ):
...
async def test_generate_insights(mock_websocket, mock_auth_session_local, mock_chat_service):
    ...
    with (
        patch("app.api.sockets.handlers.AsyncSessionLocal", new=mock_auth_session_local),
        patch("app.api.sockets.handlers.InsightGenerator", return_value=mock_generator),
        patch("app.api.sockets.handlers.ChatService", return_value=mock_chat_service),
        patch("app.api.sockets.handlers.manager.send_message", new_callable=AsyncMock) as mock_send,
        patch("app.api.sockets.handlers.send_progress", new_callable=AsyncMock),
    ):
