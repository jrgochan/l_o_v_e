...
async def test_get_path_templates_filtering(mock_db):
    ...
    resp = await get_path_templates(
        from_emotion="Anxiety", to_emotion=None, max_difficulty=None, db=mock_db
    )
    ...
    resp = await get_path_templates(
        from_emotion=None, to_emotion=None, max_difficulty=0.6, db=mock_db
    )
    assert resp["filtered_count"] == 1
...
async def test_get_challenge_patterns(mock_db):
    ...
    patterns = [{"challenge_name": "Anxiety"}, {"challenge_name": "Shame"}]
...
async def test_get_path_templates_string_parsing(mock_db):
    ...
    template_data = {"from_emotion": "Anxiety", "to_emotion": "Calm", "difficulty": 0.5}
...
async def test_get_challenge_patterns_string_parsing(mock_db):
    ...
    """
    pattern_data = {"challenge_name": "Anxiety", "description": "Test Pattern"}
...
async def test_get_path_templates_dict_content(mock_db):
    ...
    resp = await get_path_templates(
        db=mock_db, from_emotion=None, to_emotion=None, max_difficulty=None
    )
    assert resp["success"] is True
...
async def test_get_all_bootstrap_data_dict_content(mock_db):
    ...
    rows = [("strategy_effectiveness", "gen", {"rating": 5}, None)]
    mock_result = MagicMock()
    mock_result.fetchall.return_value = rows
    mock_db.execute.return_value = mock_result
