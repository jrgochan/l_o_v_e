...
def mock_deps(mock_db_session, mock_chat_service):
    ...
    with (
        ...
        patch("app.api.sockets.connection.manager.connect", new=AsyncMock()) as mock_connect,
        patch("app.api.sockets.connection.manager.disconnect") as mock_disconnect,
        patch(
            "app.api.sockets.connection.manager.send_message", new=AsyncMock()
        ) as mock_send_message,
        patch("app.api.sockets.connection.manager.send_text", new=AsyncMock()) as mock_send_text,
        patch(
            "app.api.sockets.connection.manager.get_db_session", return_value=None
        ) as mock_get_db,
        patch("app.api.sockets.connection.manager.set_db_session") as mock_set_db,
        patch("httpx.AsyncClient") as MockHttp,
        patch("app.api.sockets.handlers.InsightGenerator") as MockGen,
    ):
...
async def test_chat_loop_exception(mock_websocket, mock_deps):
    ...
    await chat_ws.chat_websocket(
        mock_websocket, "session1", current_user=MagicMock(id=uuid4(), email="test@test.com")
    )
...
async def test_handle_user_message_exception(mock_websocket, mock_deps):
    ...
    with patch(
        "app.api.sockets.handlers.process_text_message", side_effect=ValueError("Processing died")
    ):
...
async def test_handle_user_message_route_logic(mock_websocket, mock_deps):
    ...
    with (
        patch("app.api.sockets.handlers.process_text_message", new=AsyncMock()) as mock_text,
        patch("app.api.sockets.handlers.process_audio_message", new=AsyncMock()) as mock_audio,
    ):
...
async def test_process_text_message_flow(mock_websocket, mock_deps):
    ...
    with patch("app.api.sockets.handlers.generate_insights", new=AsyncMock()) as mock_gen:
...
async def test_process_text_message_existing_session(mock_websocket, mock_deps):
    ...
    with patch("app.api.sockets.handlers.generate_insights", new=AsyncMock()):
        await chat_ws.process_text_message("s1", "hello", "warm", False, mock_websocket)
        ...
        assert (
            mock_deps["chat_service"].save_user_message.call_args[1]["session_id"] == existing_uuid
        )
...
async def test_process_audio_existing_session(mock_websocket, mock_deps):
    ...
    with (
        ...
        patch("builtins.open"),
        patch("os.remove"),
        patch("app.api.sockets.handlers.generate_insights", new=AsyncMock()),
    ):
...
async def test_text_message_deep_feeling(mock_websocket, mock_deps):
    ...
    with patch(
        "app.api.sockets.handlers.handle_multi_emotion_result", new=AsyncMock()
    ) as mock_multi:
        await chat_ws.process_text_message(
            "s1", "Deep text", "warm", True, mock_websocket, "guest", None
        )
...
async def test_handle_multi_emotion_full(mock_websocket, mock_deps):
    ...
    with patch("app.api.sockets.handlers.generate_insights", new=AsyncMock()) as mock_insights:
...
async def test_handle_single_emotion_db_error(mock_websocket, mock_deps):
    ...
    with patch("app.api.sockets.handlers.generate_insights", new=AsyncMock()) as mock_insights:
        await chat_ws.handle_single_emotion_result("s1", uuid4(), result, "warm", mock_websocket)
...
async def test_process_text_db_save_failure_catch(mock_websocket, mock_deps):
    ...
    with (
        patch("app.api.sockets.handlers.generate_insights", new=AsyncMock()) as mock_gen,
        patch("app.api.sockets.handlers.handle_single_emotion_result", new=AsyncMock()),
    ):  # Mock handle_single to isolate 536 block
...
async def test_process_audio_analysis_phase_error(mock_deps, mock_websocket):
    ...
    with (
        ...
        # Act & Assert
        await chat_ws.process_audio_message(
            session_id, "base64audio", None, "warm", False, mock_websocket, user_identifier="user1"
        )
...
async def test_generate_insights_save_error(mock_deps, mock_chat_service):
    ...
    await chat_ws.generate_insights(
        db_session_id, session_id, {"emotion": "Joy", "confidence": 0.9}, "warm", None, MagicMock()
    )
