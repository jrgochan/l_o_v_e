...
async def test_get_context_recommendations_not_found(mock_db):
    ...
    with pytest.raises(HTTPException) as exc:
        await get_context_recommendations(
            time_of_day=None, energy_level=None, location=None, 
            available_time=None, experience_level=None, db=mock_db
        )
    assert exc.value.status_code == 404
    ...
    modifiers = [
        {
            ...
            "modifier_value": {
                "work": {"recommended_strategies": ["Deep Breath"]}
            }
        },
        {
            "modifier_type": "available_time",
            "modifier_value": {
                "5_minutes": {"recommended_strategies": ["Quick Scan"]}
            }
        },
        {
            "modifier_type": "experience_level",
            "modifier_value": {
                "beginner": {"recommended_strategies": ["Basic Grounding"]}
            }
        }
    ]
