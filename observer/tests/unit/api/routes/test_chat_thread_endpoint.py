...
def test_get_message_thread_success(client, mock_user, mock_chat_service):
    ...
    # Mock dependencies
    app.dependency_overrides["app.api.deps.get_current_user"] = lambda: mock_user
    ...
    # Actually, if DisplayMessage expects 'type', and ChatMessage has 'message_type', validation might fail.
    ...
    with patch("app.api.routes.chat_websocket.ChatService", return_value=mock_chat_service) as mock_service_cls:
...
def test_get_message_thread_empty(client, mock_user, mock_chat_service):
    ...
    with patch("app.api.routes.chat_websocket.ChatService", return_value=mock_chat_service):
