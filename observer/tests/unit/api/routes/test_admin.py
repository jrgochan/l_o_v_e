...
async def test_test_prompt_render():
    ...
    # Error case
    req_err = PromptTestRequest(
        template_content="Hello {age}", 
        input_variables={"name": "World"}
    )
...
async def test_get_session_details(mock_db, mock_admin_user):
    ...
    m1 = MagicMock(); m1.created_at = datetime(2023, 1, 1); m1.to_dict.return_value = {"msg": 1}
    m2 = MagicMock(); m2.created_at = datetime(2023, 1, 2); m2.to_dict.return_value = {"msg": 2}
...
async def test_admin_import_strategies_missing_name(mock_db, mock_admin_user):
    ...
    data = {"strategies": [{"description": "No Name"}, {"name": "Valid"}]}
...
async def test_import_strategies(mock_db, mock_admin_user):
    data = {
        "strategies": [
            {"name": "S1", "description": "Update me"},
            {"name": "S2", "description": "Create me"}
        ]
    }
    
    # First query returns S1, Second query returns None (for S2)
    s1 = TransitionStrategy(id=uuid.uuid4(), strategy_name="S1")
    
    # We need execute to return different things for different calls or based on query
    # Simple approach: side_effect iterable
    mock_res1 = MagicMock(); mock_res1.scalar_one_or_none.return_value = s1
    mock_res2 = MagicMock(); mock_res2.scalar_one_or_none.return_value = None
    ...
    assert res["updated"] == 1
    assert res["created"] == 1
    assert s1.description == "Update me"
    mock_db.add.assert_called() # For S2
...
async def test_import_atlas_data_partial_failure(mock_db, mock_admin_user):
    ...
    import_data = {
        "emotions": [
            {"emotion_name": "Joy", "definition": "Happy", "vac": [0.1, 0.1, 0.1], "category": "pos"}, # Success
            {"emotion_name": "Anger", "definition": "Mad", "vac": [0.1, 0.1, 0.1], "category": "neg"}  # Fail
        ]
    }
    ...
    joy_result = MagicMock(); joy_result.scalars.return_value.first.return_value = joy
    anger_result = MagicMock(); anger_result.scalars.return_value.first.return_value = anger
...
async def test_admin_import_strategies_errors(mock_db, mock_admin_user):
    ...
    mock_db.commit = AsyncMock(side_effect=Exception("DB Fail"))
    
    # Needs a dictionary, not a file, as the endpoint takes JSON body
    data = {"strategies": [{"name": "Strat1", "category": "General"}]}
...
async def test_admin_import_strategies_partial_exception(mock_db, mock_admin_user):
    ...
    assert "Error: Simulated DB Loop Error" in response["errors"][0]
