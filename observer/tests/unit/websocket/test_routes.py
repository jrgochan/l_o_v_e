...
from app.websocket.routes import websocket_endpoint, heartbeat_loop
...
async def test_websocket_pong_and_error(mock_ws, mock_db, caplog):
    ...
    with patch("app.websocket.routes.manager", new_callable=AsyncMock) as mock_manager, \
        ...
        assert error_sent, f"Error message not sent. Captured: {captured_messages}. Raw Calls: {calls}"
...
async def test_websocket_inner_loop_exception(mock_ws, mock_db, caplog):
    ...
    with patch("app.websocket.routes.manager", new_callable=AsyncMock) as mock_manager, \
        ...
        mock_ws.receive_text.side_effect = [
            ...
            # Note: The exception breaks the loop, so WebSocketDisconnect is not actually reached/needed 
...
async def test_heartbeat_loop_error(caplog):
    ...
    with patch("asyncio.sleep"), \
         patch("app.websocket.routes.manager.send_ping", side_effect=[Exception("PingFail"), asyncio.CancelledError]):
