...
class WaypointExplainer:
    ...
    async def explain_waypoint(
        ...
        Args:
            ...
            path_context: Additional context (start, goal, full path)

        Returns:
            Dictionary with explanation, VAC analysis, context, signs, citations
        """
        logger.debug(
            f"Explaining waypoint: {waypoint_emotion.emotion_name} "
            f"(from {previous_emotion.emotion_name} to {next_emotion.emotion_name})"
        )
        ...
        if template:
            logger.debug(f"Using template for {waypoint_emotion.emotion_name}")
        ...
        logger.debug(f"Generating fallback explanation for {waypoint_emotion.emotion_name}")
    ...
    def _get_vac_distance(self, vac1: List[float], vac2: List[float]) -> float:
        """Calculate Euclidean distance in VAC space."""
        import math

        return math.sqrt(sum((float(a) - float(b)) ** 2 for a, b in zip(vac1, vac2)))
