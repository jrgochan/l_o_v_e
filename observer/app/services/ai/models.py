...
class AIModelService:
    ...
    async def get_assignment_for_function(self, function: str) -> str:
        ...
        if function not in self.FUNCTIONS:
            logger.warning("Unknown function: %s. Using default model.", function)
        ...
        try:
            ...
            logger.info("No assignment for %s, using default", function)
            return self.DEFAULT_MODEL
        except Exception as e:  # pylint: disable=broad-exception-caught
            logger.error("Failed to get assignment for %s: %s", function, e)
            return self.DEFAULT_MODEL
    ...
    async def assign_model(
        ...
        try:
            ...
            logger.info("Assigned model %s to function %s", ai_model_name, function)
            ...
            raise RuntimeError(f"Failed to assign model: {str(e)}") from e
    ...
    async def update_performance_metrics(
        ...
        try:
            ...
            if assignment and assignment.ai_model_name == ai_model_name:
                ...
                assignment.last_used_at = datetime.now(timezone.utc)

                await self.db.commit()
        except Exception as e:  # pylint: disable=broad-exception-caught
            logger.error("Failed to update performance metrics: %s", e)
