...
from app.services.emotions.rules.base import RelationshipContext
from app.services.emotions.rules.definitions import (
    AmplifyingRule,
    ComplementaryRule,
    ContradictoryRule,
    MaskingRule,
...
class EmotionRelationshipService:
    ...
    def __init__(self) -> None:
        """Initialize service with relationship rules."""
        self.rules = [
            ContradictoryRule(),
            MaskingRule(),
            AmplifyingRule(),
    ...
    def analyze_relationships(self, emotions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        # Analyze all pairs
        for i, emotion_a in enumerate(emotions):
    ...
    def _classify_relationship(
        ...
        if known_key_2 in self.KNOWN_RELATIONSHIPS:
    ...
    def _infer_relationship_from_vac(
        ...
        """Infer relationship type from VAC coordinates using strategy rules.
        ...
        # Default: Weak complementary if not clearly anything else but close
        if distance < 1.0:
            return {
                "emotion_a": emotion_a["emotion_name"],
                "emotion_b": emotion_b["emotion_name"],
